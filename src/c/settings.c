...
#define SETTINGS_PERSIST_KEY 11
...
void settings_init_defaults(Settings *s) {
  ...
  s->ring_orientation = 0;      // midnight at the top of the ring
}
...
bool settings_update_from_dict(Settings *s, DictionaryIterator *iter) {
  ...
  if (apply_u16(&s->ring_orientation, dict_find(iter, MESSAGE_KEY_RingOrientation))) changed = true;
