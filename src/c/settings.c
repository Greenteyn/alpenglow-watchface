...
// Assign only when the value REALLY differs. Settings ride along in EVERY
// packet (the phone sends them next to the astronomy data), so "the key is
// present" does not mean "the user changed something". Without this check the
// settings were rewritten to persist and the auto-return timer was rearmed
// about once an hour for no reason.
